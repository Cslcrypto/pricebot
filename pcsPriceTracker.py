...
contract2 = web3.eth.contract(address=tokenAddress, abi=wbnbABI)

botToken = '5526163505:AAGVUiHoLajqO5yND4zjNq638QqoQI-LrHo'
bot = Bot(botToken)
...
def test_function(update:Update,context:CallbackContext):
    ...
    BnbLiquidity = contract.functions.balanceOf(pairAddress).call()
    CslLiquidity = contract2.functions.balanceOf(pairAddress).call()
    
    bot.send_message(chat_id=update.effective_chat.id, text=f'''
Name : Colossal Finance
Symbol : CSL
CSL Price : ${round((float(web3.fromWei(BnbLiquidity, 'ether'))/float(web3.fromWei(CslLiquidity, 'ether')))*BnbPrice, 7)}
BNB Price : ${BnbPrice}
LP Holdings : ${int(float(web3.fromWei(BnbLiquidity, 'ether'))*(BnbPrice*2))}
''')
